...
int main(int argc, char **argv)
{
  ...
  TH1F *hMCD0DecayL = new TH1F("hMCD0DecayL", "D0 decay length;L (mm)", 1000, 0, 10);
  ...
  TH1F *hNRecoVtxPart = new TH1F("hNRecoVtxPart", "Number of tracks used for reconstructing vertices;N", 50, 0, 50);
  ...
  const int nDimDca = 5;
  const int nBinsDca[nDimDca] = {100, 20, 500, 50, 50};
  const double minBinDca[nDimDca] = {0, -5, -1+0.002, 0, 0};
  const double maxBinDca[nDimDca] = {10, 5, 1+0.002, 50, 50};
  ...
  for(int i=0; i<3; i++)
    {
      for(int j=0; j<2; j++)
	{
	  hPrimTrkDcaToRCVtx[i][j] = new THnSparseF(Form("hPrim%s%sToRCVtx",part_name[i],axis_name[j]), Form("%s distribution for primary %s;p_{T} (GeV/c);#eta;%s (mm);N_{MC};N_{reco}",axis_title[j],part_title[i],axis_title[j]), nDimDca, nBinsDca, minBinDca, maxBinDca);

	  hPrimTrkDcaToMCVtx[i][j] = new THnSparseF(Form("hPrim%s%sToMCVtx",part_name[i],axis_name[j]), Form("%s distribution for primary %s;p_{T} (GeV/c);#eta;%s (mm);N_{MC};N_{reco}",axis_title[j],part_title[i],axis_title[j]), nDimDca, nBinsDca, minBinDca, maxBinDca);

	  hSecTrkDcaToRCVtx[i][j] = new THnSparseF(Form("hSec%s%sToRCVtx",part_name[i],axis_name[j]), Form("%s distribution for secondary %s;p_{T} (GeV/c);#eta;%s (mm);N_{MC};N_{reco}",axis_title[j],part_title[i],axis_title[j]), nDimDca, nBinsDca, minBinDca, maxBinDca);

	  hSecTrkDcaToMCVtx[i][j] = new THnSparseF(Form("hSec%s%sToMCVtx",part_name[i],axis_name[j]), Form("%s distribution for secondary %s;p_{T} (GeV/c);#eta;%s (mm);N_{MC};N_{reco}",axis_title[j],part_title[i],axis_title[j]), nDimDca, nBinsDca, minBinDca, maxBinDca);
	}
    }
  ...
  const int nDimMass = 4;
  const int nBinsMass[nDimMass] = {100, 10, 100, 100};
  const double minBinMass[nDimMass] = {1.6, 0, 0, 0};
  const double maxBinMass[nDimMass] = {2.0, 1, 1, 100};
  THnSparseF *hnInvMass[2][2];
  ...
  for(int i=0; i<2; i++)
      ...
      for(int j=0; j<2; j++)
	{
	  ...
	  hnInvMass[i][j] = new THnSparseF(Form("hnInvMass_%s_%s", pair_name[i], cut_name[j]), "Invariant mass of unlike-sign #piK pairs;M_{#piK} (GeV/c^{2});z;x;Q^{2}", nDimMass, nBinsMass, minBinMass, maxBinMass);
	}
    }

  TTreeReader treereader(chain);
  // global characteristics
  TTreeReaderArray<float> rc_x_Elec = {treereader, "InclusiveKinematicsElectron.x"};
  TTreeReaderArray<float> rc_Q2_Elec = {treereader, "InclusiveKinematicsElectron.Q2"};

  // reconstructed scattered electron
  TTreeReaderArray<int> scat_elec_index = {treereader, "_InclusiveKinematicsElectron_scat.index"};
  TTreeReaderArray<float> rcPartPx = {treereader, "ReconstructedParticles.momentum.x"};
  TTreeReaderArray<float> rcPartPy = {treereader, "ReconstructedParticles.momentum.y"};
  TTreeReaderArray<float> rcPartPz = {treereader, "ReconstructedParticles.momentum.z"};
  TTreeReaderArray<float> rcPartE = {treereader, "ReconstructedParticles.energy"};

  // truth scattered electron
  TTreeReaderArray<int> mc_scat_elec_index = {treereader, "MCScatteredElectrons_objIdx.index"};
  ...
  while(treereader.Next())
    {
      ...
      TLorentzVector pin_elec, pin_proton; 
      for(int imc=0; imc<nMCPart; imc++)
	{
	  if(mcPartGenStatus[imc] == 4)
	    {
	      ...
	      if(mcPartPdg[imc] == 11)
		{
		  ...
		  pin_elec.SetPxPyPzE(mcMomPx[imc], mcMomPy[imc], mcMomPz[imc], mcE);
		  //cout << "Found one" << imc << ", " << mcPartPdg[imc] << ", M = " << pin_elec.M() << endl;
		}
	      else
		{
		  pin_proton.SetPxPyPzE(mcMomPx[imc], mcMomPy[imc], mcMomPz[imc], mcE);
		  //cout << "Found another" << imc << ", " << mcPartPdg[imc] << ", M = " << pin_proton.M() << endl;
		}
	    }
	}
      hEventStat->Fill(0.5);
      ...
      // truth reconstructed electron
      TLorentzVector pscat_elec_mc(0, 0, 0, 0);
      if(mc_scat_elec_index.GetSize()>0)
	{
	  int imc = mc_scat_elec_index[0];
	  double mcE = sqrt(pow(mcMomPx[imc],2)+pow(mcMomPy[imc],2)+pow(mcMomPz[imc],2)+pow(mcPartMass[imc],2));
	  pscat_elec_mc.SetPxPyPzE(mcMomPx[imc], mcMomPy[imc], mcMomPz[imc], mcE);
	}
      ...
      if(prim_vtx_index.GetSize()>0)
	{
	  ...
	  nRecoVtxPart = vtxAssocPart_end[rc_vtx_index] - vtxAssocPart_begin[rc_vtx_index];
	}
      hNRecoVtxPart->Fill(nRecoVtxPart);
      ...
      for(int imc=0; imc<nMCPart; imc++)
	{
	  if(mcPartGenStatus[imc] == 1 && mcPartCharge[imc] != 0)
		...
		{		  
		  ...
		  if(rc_index>=0 && prim_vtx_index.GetSize()>0)
		    {
		      ...
		      if(ip>=0)
			{
			  ...
			  double fill1[] = {mom.Pt(), mom.Eta(), dca_xy, nMcPart*1., nRecoVtxPart*1.};
			  double fill2[] = {mom.Pt(), mom.Eta(), dcaToVtx.z(), nMcPart*1., nRecoVtxPart*1.};
			  hPrimTrkDcaToRCVtx[ip][0]->Fill(fill1);
			  hPrimTrkDcaToRCVtx[ip][1]->Fill(fill2);

			  double fill3[] = {mom.Pt(), mom.Eta(), dca_xy_mc, nMcPart*1., nRecoVtxPart*1.};
			  double fill4[] = {mom.Pt(), mom.Eta(), dcaToMcVtx.z(), nMcPart*1., nRecoVtxPart*1.};
      ...
      for(int imc=0; imc<nMCPart; imc++)
	{
	  if(fabs(mcPartPdg[imc]) != 3122) continue;
	  is_lambda = true;
	  printf("Lambda prod vtx = (%2.4f, %2.4f, %2.4f), decay = %d \n", mcPartVx[imc], mcPartVy[imc], mcPartVz[imc], mcPartDaughter_end[imc]-mcPartDaughter_begin[imc]);

  	  // mother kinematics
	  for(int ip=mcPartParent_begin[imc]; ip<mcPartParent_end[imc]; ip++)
	    {
	      int mc_part_index = mcPartParent_index[ip];
	      printf("mother vtx = (%2.4f, %2.4f, %2.4f), pdg = %d\n", mcPartVx[mc_part_index], mcPartVy[mc_part_index], mcPartVz[mc_part_index], mcPartPdg[mc_part_index]);
	    }

	  // decay dauther kinematics
	  for(int ip=mcPartDaughter_begin[imc]; ip<mcPartDaughter_end[imc]; ip++)
	    {
	      int mc_part_index = mcPartDaughter_index[ip];
	      printf("daughter vtx = (%2.4f, %2.4f, %2.4f), pdg = %d\n", mcPartVx[mc_part_index], mcPartVy[mc_part_index], mcPartVz[mc_part_index], mcPartPdg[mc_part_index]);
	    }
	}
      ...
      if(is_lambda)
	{
	  for(int imc=0; imc<nMCPart; imc++)
	    {
	      printf("Partile %d, pdg = %d, prod vtx = (%2.4f, %2.4f, %2.4f), decay = %d \n", imc, mcPartPdg[imc], mcPartVx[imc], mcPartVy[imc], mcPartVz[imc], mcPartDaughter_end[imc]-mcPartDaughter_begin[imc]);

	      // mother kinematics
	      for(int ip=mcPartParent_begin[imc]; ip<mcPartParent_end[imc]; ip++)
		{
		  int mc_part_index = mcPartParent_index[ip];
		  printf("     [i] mother %d, vtx = (%2.4f, %2.4f, %2.4f), pdg = %d\n", mc_part_index, mcPartVx[mc_part_index], mcPartVy[mc_part_index], mcPartVz[mc_part_index], mcPartPdg[mc_part_index]);
		}
	    }
	}
      */
      ...
      for(int imc=0; imc<nMCPart; imc++)
	{
	  ...
	  TVector3 decay_vtx(mcEndPointX[imc], mcEndPointY[imc], mcEndPointZ[imc]);
	  double decayL = (decay_vtx-vertex_mc).Mag();
	  hMCD0DecayL->Fill(decayL);

	  //printf("D0 prod vtx = (%2.4e, %2.4e, %2.4e)\n", mcPartVx[imc], mcPartVy[imc], mcPartVz[imc]);

	  //printf("D0 decay vtx = (%2.4e, %2.4e, %2.4e)\n", mcEndPointX[imc], mcEndPointY[imc], mcEndPointZ[imc]);
	  ...
	  for(int ip = 0; ip<2; ip++)
	    {
	      ...
	      //printf("daughter vtx = (%2.4f, %2.4f, %2.4f)\n", mcPartVx[mc_part_index], mcPartVy[mc_part_index], mcPartVz[mc_part_index]);
	      ...
	      if(rc_part_index>=0 && prim_vtx_index.GetSize()>0)
		{
      ...
      // skip events without reconstructed vertex
      if(CTVx.GetSize()<=0) continue;

      // find reconstructed scattered electron
      if(scat_elec_index.GetSize()<=0) continue;
      
      //cout << "Scattering electron: # = " << scat_elec_index.GetSize() << ", index = " << scat_elec_index[0] << endl;
      TLorentzVector pscat_elec;
      pscat_elec.SetPxPyPzE(rcPartPx[scat_elec_index[0]], rcPartPy[scat_elec_index[0]], rcPartPz[scat_elec_index[0]], rcPartE[scat_elec_index[0]]);
      TLorentzVector qvec = pin_elec - pscat_elec;
      TLorentzVector qvec_mc = pin_elec - pscat_elec_mc;
      //cout << "Scattered electron mass = " <<  pscat_elec.M() << ", truth = " << pscat_elec_mc.M() << endl;
      ...
      for(unsigned int i=0; i<pi_index.size(); i++)
	{
	  ...
	  for(unsigned int j=0; j<k_index.size(); j++)
	      ...
	      if(rcCharge[pi_index[i]]*rcCharge[k_index[j]]<0)
		{
		  ...
		  // ztrue = pintrue.Dot(houttrue)/pintrue.Dot(qvectrue);
		  double parent_z = pin_proton.Dot(parent)/pin_proton.Dot(qvec_mc);

		  double fill[] = {parent.M(), parent_z, rc_x_Elec[0], rc_Q2_Elec[0]};
