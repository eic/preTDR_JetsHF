...
bkg = TreeHandler('Data_Preparation/Filtered_DISSample/BkgD0.root','treeMLBkg')
...
filename = f'ML_Output_Optuna_{args.ymin}_{args.ymax}_{args.ptmin}_{args.ptmax}/sig_bkg_distributions_plot.txt'
...
features_for_train = [v for v in vars_to_draw if v not in ['mass_D0','pt_D0','d0xy_k','d0xy_pi','d0_pi','d0_k','y_D0','dca_12','sum_d0xy','decay_length','costheta_xy']]
...
hyper_pars_ranges = {'n_estimators': (100, 1000), 'max_depth': (1, 3), 'learning_rate': (0.01, 0.1)}
...
# Compute predicted probabilities for full training and test sets
train_pred_probs = model.predict_proba(X_train_sel)
test_pred_probs = model.predict_proba(X_test_sel)

# Print the shape to verify
print("Train Predicted Probabilities Shape: \n", train_pred_probs)  # Should be (len(X_train_sel), n_classes)
print("Test Predicted Probabilities Shape: \n", test_pred_probs)  # Should be (len(X_test_sel), n_classes)

# Convert to Pandas DataFrame for better visualization
train_probs_df = pd.DataFrame(train_pred_probs, columns=["Background_Prob", "Signal_Prob"])
test_probs_df = pd.DataFrame(test_pred_probs, columns=["Background_Prob", "Signal_Prob"])

# Plot train and test distributions for signal and background
plt.figure(figsize=(12, 6))

# Train set
plt.hist(train_probs_df["Signal_Prob"], bins=50, alpha=0.5, label="Train (Signal Probability)", color='blue', density=True)
plt.hist(train_probs_df["Background_Prob"], bins=50, alpha=0.5, label="Train (Background Probability)", color='red', density=True)


# Define bins
bins = np.linspace(0, 1, 50)

# Compute histograms for test signal and background probabilities
test_signal_hist, bin_edges = np.histogram(test_probs_df["Signal_Prob"], bins=bins, density=True)
test_background_hist, _ = np.histogram(test_probs_df["Background_Prob"], bins=bins, density=True)

# Compute bin centers
bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

# Compute Poisson errors (sqrt(N) scaling)
test_signal_errors = np.sqrt(test_signal_hist) / np.sum(test_signal_hist)
test_background_errors = np.sqrt(test_background_hist) / np.sum(test_background_hist)

# Plot with error bars instead of `histtype='errorbar'`
plt.figure(figsize=(12, 6))

# Test set histograms
plt.hist(test_probs_df["Signal_Prob"], bins=bins, alpha=0.5, label="Test (Signal Probability)", color='blue', density=True)
plt.hist(test_probs_df["Background_Prob"], bins=bins, alpha=0.5, label="Test (Background Probability)", color='red', density=True)

# Adding error bars
plt.errorbar(bin_centers, test_signal_hist, yerr=test_signal_errors, fmt='o', color='blue', label="Test (Signal Prob) Error")
plt.errorbar(bin_centers, test_background_hist, yerr=test_background_errors, fmt='o', color='red', label="Test (Background Prob) Error")

plt.xlabel("Probability")
plt.ylabel("Density")
plt.title("Signal and Background Probability Distributions with Error Bars")
plt.legend()
plt.savefig(f'ML_Output_Optuna_{args.ymin}_{args.ymax}_{args.ptmin}_{args.ptmax}/BDT_probability_Shyam.png')
plt.close()
