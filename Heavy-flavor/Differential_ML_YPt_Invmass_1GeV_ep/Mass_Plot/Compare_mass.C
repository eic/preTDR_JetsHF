...
void Compare_mass(){
  ...
  double nEvents = 4.72162e+9; // 10fb-1 = 4.72162e+9 ;
  ...
  int fact_rebin = 5;
    ...
    for (Int_t ipt=0; ipt<nBins-3; ++ipt) {
        ...
        if(iy==0 || iy==1) legend->SetTextSize(0.050);
    ...
    //  gPad->SetTicks(1,1);
      ...
      hMass->GetYaxis()->SetTitleOffset(1.05);
     }
      if (iy==0 ||iy==1) hMass->GetYaxis()->SetRangeUser(0.2e-6,20.0e-4);
      else hMass->GetYaxis()->SetRangeUser(0.2e-5,8.0e-4);
      hMass->Draw("EP");
     // hMass_ML->SetStats(true);  
     ...
     // legend -> AddEntry(hMass_ML,Form("w PID + BDT Thr > %1.2f",thr), "p");
      legend -> Draw();
      }
	  }
	   // c[0]->cd(3); gPad->SetRightMargin(0.01);
      ...
      Text_com.DrawLatexNDC(.18,.85,"e+p, #sqrt{s} = 63 GeV");
      Text_com.DrawLatexNDC(.18,.78,"D^{0} #it{p}_{T} > 1 GeV/c");
      c[0]->SaveAs("JetsHF_Inv_mass_wo_wML_ep.png");
      c[0]->SaveAs("JetsHF_Inv_mass_wo_wML_ep.pdf");
      c[0]->SaveAs("JetsHF_Inv_mass_wo_wML_ep.root");
