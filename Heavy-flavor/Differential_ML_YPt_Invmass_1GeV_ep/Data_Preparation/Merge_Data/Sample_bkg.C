...
void Sample_bkg(double ymin = -1.0, double ymax = 1.0, double ptmin = 1.0, double ptmax = 2.0){
...
/*
TFile *fexp_D0_Signal = TFile::Open("../Expected_reco_D0_y_pt.root");
TH2F *hexp_D0Signal = (TH2F*)fexp_D0_Signal->Get("h_DIS_Expected");
Int_t binX = hexp_D0Signal->GetXaxis()->FindBin(0.5*(ymin+ymax));
Int_t binY = hexp_D0Signal->GetYaxis()->FindBin(0.5*(ptmin+ptmax));
double nexpected_D0 = hexp_D0Signal->GetBinContent(binX,binY); 
printf("Rapidity: (%f,%f) Pt Range: (%f,%f)\n",ymin,ymax,ptmin,ptmax);
*/
// Reading Event histgram for DIS sample		
TFile *file_DISsample = TFile::Open("../../../../D0_Jan25_Bkg/HF_reco/helix/test.root");
...
double scaling_factor = (4.72162e+9)/hEvents_DISsample->GetBinContent(1); // 493857567.0/hEvents_DISsample->GetBinContent(1); // Scaling background by 100.
