...
Double_t pt_arr[nBins-2] = { 1.0, 20.0};
...
void fitD0withStudentT() {
  ...
  TCanvas *canvas[nBins][nBins-2];
  TH1D *hMassDzero[nBins][nBins-2]; // keep structure
  TH1D *hMassFit[nBins][nBins-2];   // keep structure
  TFile *file[nBins][nBins-2];
  TFile *fout[nBins][nBins-2];

  TH1F *hFitResult = new TH1F("hFitResult", "Event statistics", 4, 0, 4);
  hFitResult->GetXaxis()->SetBinLabel(1, "Signal");
  hFitResult->GetXaxis()->SetBinLabel(2, "Background");
  hFitResult->GetXaxis()->SetBinLabel(3, "Sigma");
  hFitResult->GetXaxis()->SetBinLabel(4, "errSigma");  
  ...
  for (Int_t iy=0; iy<nBins; ++iy) {
    for (Int_t ipt=0; ipt<nBins-3; ++ipt) {
      ...
      hMass->SetMaximum(hMass->GetBinContent(maxbin)*2.50);
      ...
      Double_t errwidth = fitFunc->GetParError(2);      
      ...
      leg->AddEntry((TObject*)nullptr, Form("#sigma = %.4f#pm%.4f", width,errwidth), "");
      leg->Draw();
      ...
      hFitResult->SetBinContent(3, width);
      hFitResult->SetBinContent(4, errwidth);
      fout[iy][ipt]->cd();
      hFitResult->Write();
      hMass->Write();
      sigFunc->Write();
